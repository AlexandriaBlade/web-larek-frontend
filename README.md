...
protected constructor(container: HTMLElement) - конструктор, на вход принимает контейнер, в котором необходимо создать элемент отображения. Реализуемые методы:
...
setContactField(field: keyof IContact, value: string) - заполнение полей формы контакта. Используемые типы данных:
...
IOrder - интерфейс структуры заказа. Включает в себя информацию о способе оплаты, адресе доставки, электронной почте, номере телефона, общей стоимости и списка заказанных товаров; IDelivery - интерфейс для определения полей формы заказа, связанные с данными о доставке; FormErrors - интерфейс структуры хранения ошибок валидации форм; 
...
Этот класс наследует конструктор и методы от класса Model. Вот его свойства:
...
image: string - изображение проудкта. Используемые типы данных:
IProduct - интерфейс данных товара.
```
...
protected _basket: HTMLElement; - корзина товаров. Реализуемые методы:
...
set locked(value: boolean) - сеттер блокировки страницы на основе переданного значения (необходимо при открытии модальных окон). Используемые типы данных:
...
protected _buttonTitle: string; - название кнопки; Реализуемые методы:
...
classByCategory(value: string): string - задает цвет для категории карточки. Используемые типы данных:
...
toggleButton(disabled: boolean) - активация/деактивация кнопки. Используемые типы данных:
...
protected totalAmount: HTMLElement; - сообщение о деталях успешной операции. Реализуемые методы:
set total(value: string) - сеттер собщения об итоговой стоимости операции. Используемые типы данных:
...
protected errorContainer: HTMLElement - элемент отображения ошибок валидации формы. Реализуемые методы:
