...
import { IAppState, CatalogChangeEvent, IProduct } from "./components/AppData";
...
const appData = new IAppState({}, events);
...
events.on('contacts:submit', () => {
  api.orderProducts(appData.order)
    ...
    .catch(error => {
      console.error(error);
      // Возможно, вызвать уведомление о сбое
    });
});
...
events.on('preview:changed', (item: IProduct) => {
  const card = new Card(cloneTemplate(templates.cardPreview), {
    onClick: () => {
      ...
      card.buttonElement = appData.basket.includes(item) ? 'Удалить из корзины' : 'Купить';
    }
  });
  modal.render({
    content: card.render({
...
events.on('payment:toggle', (target: HTMLElement) => {
  if (!target.classList.contains('button_alt-active')) {
    ...
    const paymentMethodName = target.getAttribute('name');
    
    if (paymentMethodName && PaymentMethods[paymentMethodName]) {
      appData.order.payment = PaymentMethods[paymentMethodName]; // Установка способа оплаты
    }
  }
});
...
api.getProductList()
  .then(products => {
    appData.setCatalog(products); // Установка каталога товаров в модель данных
  })
  .catch(error => {
